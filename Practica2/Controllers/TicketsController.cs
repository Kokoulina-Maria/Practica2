...
namespace Practica2.Controllers
{
    public class TicketsController : Controller
    {
        ...
        static public List<Practica2.Models.Ticket> list = _DataManager.TR.Tickets().ToList();
        static public List<Practica2.Models.Ticket> nowlist = _DataManager.TR.Tickets().ToList();
        static public List<Practica2.Models.Ticket> starlist = new List<Ticket>();

        // GET: Tickets
        public ActionResult Index()
        {
            list = _DataManager.TR.Tickets().ToList();
            nowlist = _DataManager.TR.Tickets().ToList();
            starlist = new List<Ticket>();
            return View(_DataManager.TR.Tickets());
        }
        ...
        {
            if (ok) nowlist = _DataManager.TR.Find((List<Ticket>)nowlist, entity, atribut, sign, text, text);
            else
            {
                ((List<Ticket>)starlist).AddRange(nowlist);
                nowlist = _DataManager.TR.Tickets().ToList();
                nowlist = _DataManager.TR.Find(nowlist, entity, atribut, sign, text, text);
            }
        ...
        {
            List<Ticket> ex = new List<Ticket>();
            if (back == "Input")
                ex = _DataManager.TR.Tickets().ToList();
            else
                ex.AddRange(list);
            ...
            string save = "Билеты" + DateTime.Now.Year + DateTime.Now.Month + DateTime.Now.Day + DateTime.Now.Hour + DateTime.Now.Minute + DateTime.Now.Second;
